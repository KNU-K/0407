    ...
    const fetchCardData = async () => {
      try {
        ...
        const response = await axios.get(
          `https://api.g-start-up.com/service1/api/card-news/${cardId}`,
          {
            headers: {
              Authorization: token,
            },
  ...
  const handleLikeClick = async () => {
    try {
      ...
      if (!liked) {
        // 좋아요 추가
        await axios.post(
          `https://api.g-start-up.com/service1/api/card-news/${cardId}/like`,
          null,
          {
            headers: {
              Authorization: token,
        ...
        <div>
          ...
          >
            {cardData.content.images.map((image, index) => (
              ...
              >
                <img
                  src={`https://www.k-startup.go.kr${image}`}
                  alt={`Image ${index + 1}`}
                  style={{
                    maxWidth: "800px",
                    maxHeight: "800px",
          ...
          <StyledPaper>
            <Typography
              variant="body1"
              dangerouslySetInnerHTML={{ __html: cardData.content.notice }}
            />
            {/* 좋아요 표시 */}
